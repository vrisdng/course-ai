...
3. **Sign in/up** — `src/pages/Auth.tsx`: `supabase.auth.signInWithPassword` / `supabase.auth.signUp`. Signup sends an email confirmation link (`emailRedirectTo`), not an OTP.
   - **Password reset** — "Forgot password?" on the sign-in tab (or `/auth?mode=forgot`) calls `supabase.auth.resetPasswordForEmail(email, { redirectTo: <origin>/reset-password })`. The emailed link lands on `src/pages/ResetPassword.tsx`, a public route; the Supabase client exchanges the link for a session on load, and the page calls `supabase.auth.updateUser({ password })`. If no session results (expired/used link) the page shows the error from the URL and offers to request a new link. **Ops note:** `<origin>/reset-password` must be listed under Auth → URL Configuration → Redirect URLs in the Supabase dashboard for every environment (localhost:8080, Vercel previews, production) or Supabase will fall back to the Site URL and the flow breaks.
...
2. ~~**No password-reset flow.**~~ Resolved — see "How it works" §3. Forgot-password + `/reset-password` shipped (`Auth.tsx`, `ResetPassword.tsx`).
