...
For the next developer picking this up. For *what* the app does, see [README.md](README.md). For *what's broken/risky*, see [HANDOFF.md](HANDOFF.md) — read that before touching auth, `AdminDashboard.tsx`, or `rag-chat`.
...
**Reminder**: `reap-stale-jobs` is meant to run on a schedule (stale-job cleanup, 5-minute staleness threshold, 5-attempt retry cap — see README's dynamic-behavior table). No `pg_cron` schedule was found checked into migrations — confirm whether this is wired up via the Supabase dashboard's cron scheduler before assuming stuck jobs self-heal in whatever environment you're working in.
